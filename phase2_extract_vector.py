...
def load_codi_model(ckpt_dir: pathlib.Path, bundle_dir: pathlib.Path, bf16: bool):
    ...
    if ckpt_path:
        ...
        if missing:
            print(f"[Phase 2]   ⚠ Missing keys: {len(missing)} (this is normal for LoRA)")
        if unexpected:
            print(f"[Phase 2]   ⚠ Unexpected keys: {len(unexpected)}")
    else:
        print(f"[Phase 2] ⚠ No checkpoint found at {ckpt_dir}, using pretrained weights from HF")
